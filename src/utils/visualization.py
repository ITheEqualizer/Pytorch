...
_NONINTERACTIVE_BACKENDS = {"agg", "pdf", "ps", "svg", "cairo", "template"}


def _maybe_show() -> None:
    """Display the current figure only when an interactive backend is active."""
    if plt.get_backend().lower() not in _NONINTERACTIVE_BACKENDS:
        plt.show()
...
def plot_learning_rate(
    lr_history: List[float], save_path: Optional[Path] = None, figsize: tuple = (10, 5)
) -> None:
    """
