...
def load_model(
    model_path: Union[str, Path], device: torch.device, model_config: ModelConfig
) -> nn.Module:
    """
    ...
    load_checkpoint(Path(model_path), model)
    model.to(device)
    model.eval()
...
def predict(
    model: nn.Module, input_data: torch.Tensor, device: torch.device
